...
parser.add_argument('--nSample', type=int, default=150, help='number features to generate per class')#900
...
def train():
    ...
    netD_optimizer = optim.Adam(netD.parameters(), lr=0.0001, betas=(0.5, 0.999))
    ...
    for it in range(start_step, opt.niter+1):
        ...
        for iter_d in range(opt.critic_iter):
            ...
            D_cost = real_ins_contras_loss+ rec_att_loss*0.1
            D_cost.backward()
            optimizerMap.step()
            cls_optimizer.step()
            optimizerDis.step()
        ...
        cls_out = classifier(outz_fake)
        fake_ins_contras_loss = opt.ins_weight*cls_criterion(cls_out, labels)
        ...
        dec_fake_att = netD(torch.cat((embed_fake_all,all_attr),1))
        ...
        if it % opt.evl_interval == 0 and it >= 5000:
            ...
            if opt.zsl:
                ...
                cls = classifier2.CLASSIFIER(opt, gen_feat, gen_label,netMap,opt.embedSize, dataset, dataset.test_seen_feature,
                                             ...
                                             dataset.ntest_class, True, opt.classifier_lr, 0.5, 40, 150, False)
                ...
                cls = classifier2.CLASSIFIER(opt, train_X, train_Y,netMap,opt.embedSize, dataset,  dataset.test_seen_feature,  dataset.test_unseen_feature,
                                    dataset.ntrain_class + dataset.ntest_class, True, opt.classifier_lr, 0.5, 30, 150, True)
