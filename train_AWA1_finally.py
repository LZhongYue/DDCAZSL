...
parser.add_argument('--manualSeed', type=int, default=27628, help='manual seed')
parser.add_argument('--input_dim',     type=int, default=85, help='dimension of the global semantic vectors')

parser.add_argument('--prototype',    type=float, default=10, help='weight of the prototype loss')
parser.add_argument('--cprototype',    type=float, default=1, help='weight of the prototype loss')
parser.add_argument('--cons',    type=float, default=1, help='weight of the prototype loss')
...
#parser.add_argument('--cls_map_lr', type=float, default=0.05, help='weight of the score function when learning G')
parser.add_argument('--map_lr', type=float, default=0.001, help='weight of the classification loss when learning G')
...
def train():
    ...
    out_dir = 'out/{}/s_d-{}_mask-{}_cl-{}_ns-{}_wd-{}_lr-{}_nS-{}_bs-{}_ps-{}'.format(opt.dataset, opt.input_dim, opt.dropout, opt.num_coupling_layers,
    ...
    for it in range(start_step, opt.niter+1):
        ...
        loss_all =    loss + G_cost + rec_fake_att_loss + fake_ins_contras_loss
        loss_all.backward()
        ...
        if it % opt.disp_interval == 0 and it:
            log_text = 'Iter-[{}/{}]; loss: {:.3f};real_ins_contras_los: {:.3f};fake_ins_contras_loss: {:.3f}; '.format(it, opt.niter, loss.item(),real_ins_contras_loss.item(),fake_ins_contras_loss.item())
        ...
        if it % opt.evl_interval == 0 and it > 0:
            ...
            if opt.zsl:
                ...
                cls = classifier2.CLASSIFIER(opt, train_X, train_Y,netMap,opt.embedSize, dataset,  dataset.test_seen_feature,  dataset.test_unseen_feature,
                                    dataset.ntrain_class + dataset.ntest_class, True, opt.classifier_lr, 0.5, 30, 1200, True)
