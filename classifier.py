...
class CLASSIFIER:
    # train_Y is interger
    def __init__(self, opt, _train_X, _train_Y, map_net, embed_size,data_loader, test_seen_feature,test_unseen_feature, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20,
        ...
        self.model =  LINEAR_LOGSOFTMAX(self.embed_size, self.nclass)
        ...
        if generalized:
            self.acc_seen, self.acc_unseen, self.H,self.preds_s,self.preds_u = self.fit()
        else:
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            ...
            acc_seen,preds_seen_label = self.val_gzsl(self.test_seen_feature, self.test_seen_label)
            acc_unseen,preds_unseen_label = self.val_gzsl(self.test_unseen_feature, self.test_unseen_label+self.ntrain_class)
            ...
            if H >= best_H:
                ...
                best_H = H
                best_seen_label = preds_seen_label
                best_unseen_label = preds_unseen_label
        return best_seen * 100, best_unseen * 100, best_H *100, best_seen_label,best_unseen_label
    ...
    def val_gzsl(self, test_X, test_label):
        ...
        return acc, predicted_label.numpy()
