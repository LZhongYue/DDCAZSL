...
parser.add_argument('--validation', action='store_true', default=True, help='enable cross validation mode')
...
parser.add_argument('--classifier_lr', type=float, default=0.005, help='learning rate to train softmax classifier')
parser.add_argument('--batchsize', type=int, default=256, help='input batch size')
parser.add_argument('--nSample', type=int, default=800, help='number features to generate per class')#900
...
parser.add_argument('--pi', type=float, default=0.03, help='degree of the perturbation')
parser.add_argument('--dropout', type=float, default=0.0, help='probability of dropping a dimension')

parser.add_argument('--ins_weight', type=float, default=0.1, help='weight of the classification loss when learning G')
parser.add_argument('--cls_weight', type=float, default=0.01, help='weight of the score function when learning G')

parser.add_argument('--nclass_all', type=int, default=102, help='number of all classes')
parser.add_argument('--nclass_seen', type=int, default=82, help='number of all classes')

parser.add_argument('--critic_iter', type=int, default=3, help='critic iteration, following WGAN-GP')
...
def train():
    ...
    classifier_att.apply(weights_init)
    ...
    optimizer = optim.Adam(flow.trainable_parameters, lr=opt.lr, weight_decay=opt.weight_decay)
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, gamma=0.8, step_size=15)
    cls_optimizer = optim.Adam(classifier.parameters(), lr=0.005, betas=(0.5, 0.999))
    cls_att_optimizer = optim.Adam(classifier_att.parameters(), lr=0.005, betas=(0.5, 0.999))
    optimizerMap = optim.Adam(netMap.parameters(), lr=0.0001, betas=(0.5, 0.999))
    netD_optimizer = optim.Adam(netD.parameters(), lr=0.001, betas=(0.5, 0.999))
    optimizerD = optim.Adam(netDis.parameters(), lr=opt.lr_d, betas=(0.5, 0.999))
    with open(log_dir, 'a') as f:
        f.write('\n')
        f.write('时间开始加速了 Training Start:')
        f.write(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime()) + '\n')
    start_step = 0
    ...
    for it in range(start_step, opt.niter+1):
        ...
        #print("all_attr", all_attr.shape)
        all_label = torch.arange(0, dataset.ntrain_class + dataset.ntest_class, dtype=torch.long).cuda()
        #train_att = torch.from_numpy(dataset.train_att).cuda()
        train_label  = torch.arange(0, dataset.ntrain_class , dtype=torch.long).cuda()
        ...
        for iter_d in range(opt.critic_iter):
            ...
            z = torch.randn(opt.batchsize, 2048).cuda()
            ...
            #class LOSS
            #dec_att= netD(embed_real)
            #cls_out_att = classifier_att(torch.cat((dec_att,all_attr),dim=0))
            cls_out_att = classifier_att(all_attr)
            #rec_att_loss = mse(dec_att,torch.from_numpy(dataset.train_att).cuda())
            rec_att_loss = cls_criterion(cls_out_att,all_label)
        ...
        fake_ins_contras_loss = opt.ins_weight*cls_criterion(cls_out_fake, labels)

        #class Loss
        z = torch.zeros(opt.nclass_all, 2048).cuda()
        fake_all, _ = flow.reverse_sample(z, all_attr)
        embed_fake_all, outz_fake_all = netMap(fake_all)
        dec_fake_att = netD(embed_fake_all)
        #rec_fake_att_loss = opt.cls_weight*mse(dec_fake_att,C)
        cls_out_fake_att = classifier_att(dec_fake_att)
        rec_fake_att_loss = opt.cls_weight*cls_criterion(cls_out_fake_att,all_label)
        loss_all =  rec_fake_att_loss + fake_ins_contras_loss
        loss_all.backward()
        netD_optimizer.step()
        optimizer.step()
        if it % iters == 0:
        ...
        if it % opt.disp_interval == 0 and it:
            log_text = 'Iter-[{}/{}]; loss: {:.3f};real_ins_contras_loss: {:.3f};'.format(it, opt.niter, loss.item(),real_ins_contras_loss.item(),fake_ins_contras_loss.item())#,rec_att_loss.item(),rec_fake_att_loss.item())
            log_print(log_text, log_dir)


        if it % opt.evl_interval == 0 and it >= 0:
            #print("cls_out", cls_out.min())
            flow.eval()
            netMap.eval()
            ...
            if opt.zsl:
                # ZSL
                cls = classifier2.CLASSIFIER(opt, gen_feat, gen_label,netMap,opt.embedSize, dataset, dataset.test_seen_feature,
                                             dataset.test_unseen_feature,
                                             dataset.ntest_class, True, opt.classifier_lr, 0.5, 40, 1000, False)
                ...
                cls = classifier2.CLASSIFIER(opt, train_X, train_Y,netMap,opt.embedSize, dataset,  dataset.test_seen_feature,  dataset.test_unseen_feature,
                                    dataset.ntrain_class + dataset.ntest_class, True, opt.classifier_lr, 0.5, 30, 600, True)
